...
import { RandomArtTaskCall } from "./RandomArtTaskCall.js"
...
export class OldRandomArtTaskRepository implements IRandomArtworkRepository {
   ...
   public async create(
      ...
      const retVal = new RandomArtTaskCall(
         rootCidStr,
         prefixBits,
      ...
      // new RandomArtTaskCall(assets.rootFolder.cid)
      // )
      // retVal.commit()
   ...
   public save(task: RandomArtTaskCall): void {
      const cid = task.cid
      this._storeCache[cid] = task
   }

   public getByCid(cid: string | CID): RandomArtTaskCall {
      let cidObj: CID
      let cidStr: string
      ...
      if (
         assets.rootFolder === undefined ||
         assets.rootFolder.toString() !== cidStr
      ) {
   ...
   public getAll(): RandomArtTaskCall[] {
