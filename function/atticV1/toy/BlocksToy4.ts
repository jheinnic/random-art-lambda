...
import {
   IpfsModuleTypes,
   SharedArtBlockstoreModuleTypes,
} from "../ipfs/di/typez.js"
...
const schemaDsl = `type ModelEnvelope union {
  | RandomArtTaskCall "RandomArtTask_1.0.0"
} representation envelope {
  discriminantKey "version"
  contentKey "model"
}

type RandomArtTaskCall struct {
  prefix Bytes
  suffix Bytes
  plotMap Link
...
export interface RandomArtTaskCall {
   prefix: Uint8Array
   suffix: Uint8Array
   plotMap: CID
}
...
export class AppService {
   ...
   public async doWork(): Promise<void> {
      ...
      for (modelLine of modelLines) {
         ...
         const raTask = await this.repository.create(
            (builder: IRandomArtTaskBuilder) => {
