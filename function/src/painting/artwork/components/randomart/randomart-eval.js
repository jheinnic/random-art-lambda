...
function getEvaluator(formula) {
  ...
  // Include all helper functions in the generated function body
  // since new Function() creates a function in global scope without access to module scope
  const helpers = `
    function average(c1, c2, w = 0.5) {
      let r1, g1, b1; [r1, g1, b1] = c1;
      let r2, g2, b2; [r2, g2, b2] = c2;
    ...
    function well(x) { return (1 - (2 / Math.pow((1 + (x * x)), 8))); }
    function tent(x) { return (1 - (2 * Math.abs(x))); }
    function modulo(x,y) { return x - y * Math.floor(x/y); }
    function BW(v) { return [v,v,v] }
    function Const(r,g,b) { return [r,g,b] }
    function RGB(c1,c2,c3) { return [brightness(...c1), brightness(...c2), brightness(...c3)] }
    function Sum(c1,c2) { return average(c1,c2) }
  ...
  return new Function('x', 'y', `${helpers}\nreturn ${formula}`);
}
