...
// Custom toFixed implementation to avoid genjs6.js prototype pollution
function fixStr(n) {
  // Handle edge cases
  if (!Number.isFinite(n)) return String(n);

  // Round to 4 decimal places without using toFixed
  ...
  if (decimals >= 4) {
    // Already has enough decimals, truncate
    return str.substring(0, dotIndex + 5);
  }

  // Pad with zeros
  return str + '0'.repeat(4 - decimals);
}
